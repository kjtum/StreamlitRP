...
while True:
    try:
        col_row = st.columns(2, gap="small")  # 1行に 2 人
        for c in col_row:
            m = next(men_iter)
            with c:
                pic, prefs_col = st.columns([0.2, 0.8], gap="small")
                with pic:
                    mp = os.path.join(IMAGE_DIR, f"{m}.png")
                    st.image(mp if os.path.exists(mp) else None, width=40, caption="" if os.path.exists(mp) else m)
                with prefs_col:
                    p = st.multiselect(m, WOMEN, default=st.session_state.men_prefs[m], key=f"men_{m}")
                    if len(p) == len(WOMEN):
                        st.session_state.men_prefs[m] = p
    except StopIteration:
        break

# --- 女性側 ---
women_iter = iter(WOMEN)
while True:
    try:
        col_row = st.columns(2, gap="small")
        for c in col_row:
            w = next(women_iter)
            with c:
                pic, prefs_col = st.columns([0.2, 0.8], gap="small")
                with pic:
                    wp = os.path.join(IMAGE_DIR, f"{w}.png")
                    st.image(wp if os.path.exists(wp) else None, width=40, caption="" if os.path.exists(wp) else w)
                with prefs_col:
                    p = st.multiselect(w, MEN, default=st.session_state.women_prefs[w], key=f"women_{w}")
                    if len(p) == len(MEN):
                        st.session_state.women_prefs[w] = p
    except StopIteration:
        break

for w in WOMEN:
    row = st.columns([0.35, 0.65])
    with row[0]:
    ...
    with row[1]:
        prefs = st.multiselect(
            f"{w}", MEN, default=st.session_state.women_prefs[w], key=f"women_{w}")
